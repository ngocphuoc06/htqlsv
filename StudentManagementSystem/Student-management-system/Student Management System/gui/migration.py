...
def step_create_enrollment_v2(cursor, conn):
    if _table_exists(cursor, 'enrollments_v2') or (_table_exists(cursor, 'enrollments') and _column_exists(cursor, 'enrollments', 'section_id')):
        return
    # Ensure students.mssv has an index so it can be referenced by a FK.
    # The students table uses 'username' as PK, so mssv needs its own UNIQUE index.
    cursor.execute("""
        SELECT COUNT(*) AS n FROM information_schema.STATISTICS
        WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='students'
          AND COLUMN_NAME='mssv'
    """)
    if cursor.fetchone()['n'] == 0:
        cursor.execute("ALTER TABLE students ADD UNIQUE INDEX idx_students_mssv (mssv)")
        conn.commit()
    cursor.execute("""
